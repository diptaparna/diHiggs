...
# version 1.1 (July 9, 2014)
# made by Beranger Dumont
# based on toy MC code by Benjamin Fuks, Chris Wymant and Sam Bein
# v1.1: BF added an efficiency map calculator
# v1.2: UL added binomial statistical uncertainties on the efficiency
...
def exclusion_check(crosssection):
    ...
    bestCLs=CLs(nobs, nb, deltanb, nsignal, numtoyexperiments)
...
for child in info_root:
    ...
    if child.tag == "region" and \
      ("type" not in child.attrib or child.attrib["type"] == "signal"):
        ...
        signalregions[regionid] = {"acceff": 0, "syst":0, "stat":0} # initialize efficiency to 0
        for rchild in child:
    ...
    for regionid in signalregions:
        ...
        for individualSR in regionid.split(";"):
            ...
            if individualSR == assoc_SR:
                ...
                # statistical errors are binomial errors on the acceff
                eps = finalnum / initialnum
                signalregions[regionid]["acceff"] += eps
                signalregions[regionid]["stat"] += math.sqrt(eps*(1-eps)/initialnum)
...
# Efficiencies
out = open('efficiencies_'+analysis_name+'.dat','w')
Title=['Signal region', 'Efficiency', 'Stat. error', 'Syst. error', 'Total error', 'Best region']
out.write(Title[0].center(50) + Title[1].center(16) + Title[2].center(21) + Title[3].center(16) +\
    Title[4].center(18)+ Title[5].rjust(10)+'\n')
for region in signalregions:
  signalregions[region]["syst"] = float(signalregions[region]["syst"])
  IsBest=False;
  if region is bestSR:
   IsBest=True
  out.write(region.replace(' ','').ljust(50) + "%1.5f".center(15) %(signalregions[region]["acceff"]) +\
     "%1.6f".center(15) %(signalregions[region]["stat"])+ \
     "%1.6f".center(15) %(signalregions[region]["syst"])+ \
     "%1.6f".center(15) %(math.sqrt(signalregions[region]["syst"]**2+signalregions[region]["stat"]**2))+\
     str(int(IsBest)).rjust(8) +\
     '\n')
out.close()
